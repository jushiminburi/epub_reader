...
class IRReaderCenterViewController: IRBaseViewcontroller, UIGestureRecognizerDelegate {
    ...
    init(withPath path:String, unzipPath: String, window: UIWindow) {
        self.window = window
    ...
    func parseBook() {
        ...
        DispatchQueue.global().async {
            if let bookMeta = try? FREpubParser().readEpub(epubPath: self.bookPath, unzipPath: self.unzipPath) {
                ...
                DispatchQueue.main.async {
                    ...
                    HUD.flash(.label("Phân tích cú pháp không thành công, hãy xem các cuốn sách khác"), delay: 1) { _ in
